  ...
  describe "with cloud storage" do
    ...
    describe 'storage' do
      ...
      it "allows for the file url to be configured by the storage" do
        @field.value = path
        @field.url.must_match %r[^https://media.example.com/0*#{@instance.id}/00001/1de7e8/vimlogo.pdf$]
        @storage.url_mapper = ->(path) { "https://cdn.example.com#{path}" }
        @field.url.must_match %r[^https://cdn.example.com/0*#{@instance.id}/00001/1de7e8/vimlogo.pdf$]
      end
      ...
      it 'falls back to the site’s default if initialized before storage_name output was defined' do
        ...
        @field.url.must_match %r[^/0*#{@instance.id}/00001/1de7e8/vimlogo.pdf$]
      end
